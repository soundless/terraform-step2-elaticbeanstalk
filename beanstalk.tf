...
resource "aws_elastic_beanstalk_application" "api1" {
    name = "test-api1"
    description = "REST api1"
}

resource "aws_elastic_beanstalk_environment" "api1" {
    name = "api1"
    application = "${aws_elastic_beanstalk_application.api1.name}"
...
output "api1_cname" {
    value = "${aws_elastic_beanstalk_environment.api1.cname}" 
}
